...
export class Lexer {
  ...
  // Avanza 1 caracter y actualiza línea/columna si hay salto
  avanzarContando() {
    const ch = this.texto[this.pos] ?? "\0";
    this.pos++;
    if (ch === "\n") {
      this.linea++;
      this.columna = 1;
  ...
  // Analizamos
  // =====================================
  analizar(textoEntrada) {
    // Permitir uso con o sin parámetro (frontend/CLI/servidor)
    if (typeof textoEntrada === "string") {
      this.texto = textoEntrada;
    } else if (this.texto == null) {
      // si nadie lo cargó antes, evitar undefined
      this.texto = "";
    }
    ...
    while (this.pos < this.texto.length) {
      ...
      if (char === "/" && this.texto[this.pos + 1] === "/") {
        // Saltar hasta antes del salto de línea (no consumimos el \n; lo manejará el while)
        this.avanzar(); // '/'
        this.avanzar(); // '/'
        while (this.pos < this.texto.length && this.texto[this.pos] !== "\n") {
          this.avanzar(); // aquí no contamos línea porque no habrá '\n'
        }
        continue;
      }

      if (char === "/" && this.texto[this.pos + 1] === "*") {
        // Saltar hasta '*/' (aquí sí contamos saltos de línea dentro del comentario)
        this.avanzar(); // '/'
        this.avanzar(); // '*'
        let cerrado = false;
        while (this.pos < this.texto.length) {
          ...
          // avanzar contando si hay \n
          this.avanzarContando();
        }
        if (!cerrado) {
          this.errors.push(
            new Error(
  ...
  recorrerIdentificador() {
    ...
    if (esReservada) {
      ...
      this.tokens.push(
        new Token("IDENTIFICADOR", buffer, this.linea, inicioCol)
      );
    }
  }
  ...
  recorrerNumero() {
    ...
    // ==================================================
    this.tokens.push(
      new Token(esDecimal ? "DOUBLE" : "INT", buffer, this.linea, inicioCol)
    );
  }
  ...
  recorrerCadena() {
    ...
    if (this.pos >= this.texto.length) {
      this.errors.push(
        new Error("Léxico", buffer, "Cadena sin cerrar", this.linea, inicioCol)
      );
      return;
    }
    this.avanzar();
  ...
  recorrerSimbolo() {
    ...
    if (
      ...
      this.tokens.push(
        new Token("OPERADOR", char + next, this.linea, inicioCol)
      );
      this.avanzar();
      this.avanzar();
      return;
  ...
  } // ahora permite '_' como primer char
  esDigito(c) {
    return /[0-9]/.test(c);
  }
}
